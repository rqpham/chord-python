...
class FingerTable:
	...
	def __init__(self, nodeID):
		...
		if nodeCount == 0:
			for x in range (1,9):
				self.table[x] = nodeID              #this makes it an array of *nodeIDs*
				self.nodeTable[x] = self            #this makes it an array of nodes
		else:
			for x in range(1,9):
				self.table[x] = nodeID+(2**(x-1))   #this makes it an array of *nodeIDs*
				self.nodeTable[x] = self            #this makes it an array of nodes
...
def join(node):
	...
	if nodeCount == 1:
		...
		#join by giving it a random node in the topology. for beginning implementation we will continue to use the first node.		
		#implement join here
